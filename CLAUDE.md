...
A production-ready **Strapi v5** CMS template with:
- Docker containerization
- PostgreSQL database
- Automated S3 backups (Cloudflare R2)
- Pre-commit linting with Vite+ (Oxlint/Oxfmt)
...
This project uses **Vite+** (`vite-plus`) for linting, formatting and typecheck (config in `vite.config.ts`):
...
Uses **Conventional Commits** (required for semantic versioning):
...
Installed once per clone with `vp config` (writes `.vite-hooks/`):

1. **pre-commit**: Runs `vp check --fix` on staged files (declared under `staged:` in `vite.config.ts`)
...
1. Creates PostgreSQL database dump (`pg_dump`)
...
2. **Run `vp check` before committing** - Pre-commit hook does this automatically
...
6. **PostgreSQL only** - Database config only supports PostgreSQL
